...
WaldA=function(gamma,alpha,beta,delta,x,q,y,eps,IVX,T){
  ...
  if(IVX){
    ...
    z=Instrument(x,delta,T)
    
    #Get IVX Waldstatistic (W(beta=0)_ivx )
    WIVX=WaldIVX(x,y,z,T)
...
SupWA=function(alpha,beta,rho,phi,sigma,delta,IVX,c,T,gam){
  ...
  eps <- mvtnorm::rmvnorm(n = T, mean = rep(0, 3), sigma = sigma)
  
  #simulate AR(1) process for nu with ar coefficiente rho
  nu=rep(NA,T)
  nu[1]=0                                               #initialize at zero
  for (i in 2:T){
    nu[i]=rho*nu[i-1]+eps[i-1,2]                        # since i starts at 2 , start with eps[i-1,2] to get first entry
  }
  ...
  y=rep(NA,T)
  for(i in 1:T){
    y[i]=(alpha[1]+beta[1]*x[i]+eps[i,1])*I1_dgp[i]+(alpha[2]+beta[2]*x[i]+eps[i,1])*I2_dgp[i]
  }
  ...
  WaldAs=sapply(gamma_grid,FUN=WaldA,alpha=alpha,beta=beta,delta=delta,x=x,y=y,q=q,eps=eps,IVX=IVX,T=T)
