...
setwd("")
#--------------------------------------------------------------------------------------------------------------------------------------------
q_all<-kable(quantiles,format="latex",caption = "Table:2 Critical values of SupWaldA using DGP4")%>%
  kable_styling()%>%
  pack_rows(group_label = "$T=200$",1,6)%>%
  pack_rows(group_label = "$T=400$",7,12)
 
cat(q_all, sep="\n", file="Crit_A_cov2.Rmd")
render("Crit_A_cov2.Rmd",output_format = "pdf_document")
#--------------------------------------------------------------------------------------------------------------------------------------------
setwd("")
...
#--------------------------------------------------------------------------------------------------------------------------------------------
#PDF Output tab
s_ges<-kable(size ,format="latex",caption = "Size properties SupWald A for DGP 4")%>%
  kable_styling()%>% add_header_above(c(" "=1,"T=200" = 3,"T=400" = 3),line_sep = 3,line = FALSE)
cat(s_ges, sep="\n", file="Size_A_DGP4.Rmd")
render("Size_A_DGP4.Rmd",output_format = "pdf_document")
#--------------------------------------------------------------------------------------------------------------------------------------------
setwd("")
