...
#--------------------------------------------------------------------------------------------------------------------------------------------
#PDF Output tab
q_200<-kable(quantiles_200, format="latex",caption = "DGP3 T=200")
cat(q_200, sep="\n", file="Crit_A_200_cov1.Rmd")
render("Crit_A_200_cov1.Rmd",output_format = "pdf_document")
#--------------------------------------------------------------------------------------------------------------------------------------------
#PDF Output tab
q_400<-kable(quantiles_400,format="latex",caption = "DGP3 T=400")
cat(q_400, sep="\n", file="Crit_A_400_cov1.Rmd")
render("Crit_A_400_cov1.Rmd",output_format = "pdf_document")
#--------------------------------------------------------------------------------------------------------------------------------------------
q_all<-kable(quantiles,format="latex",caption = "Table:1 Critical values of SupWaldA using DGP3")%>%
  kable_styling()%>%
  pack_rows(group_label = "$T=200$",1,6)%>%
  pack_rows(group_label = "$T=400$",7,12)
 
cat(q_all, sep="\n", file="Crit_A_cov1.Rmd")
render("Crit_A_cov1.Rmd",output_format = "pdf_document")
#--------------------------------------------------------------------------------------------------------------------------------------------
...
#--------------------------------------------------------------------------------------------------------------------------------------------
#PDF Output tab
s_ges<-kable(size ,format="latex",caption = "Size properties SupWald A for DGP 3")%>%
  kable_styling()%>% add_header_above(c(" "=1,"T=200" = 3,"T=400" = 3),line_sep = 3,line = FALSE)
cat(s_ges, sep="\n", file="Size_A_DGP3.Rmd")
render("Size_A_DGP3.Rmd",output_format = "pdf_document")
#--------------------------------------------------------------------------------------------------------------------------------------------
setwd("C:/Users/peerl/OneDrive/Desktop/Studium/Master 4/Seminar/Simulation/Clean")